...
void Display::put_text(int x, int y, const char *content) const {
    ...
    // Set the cursor to the new adjusted position to center the modified text
...
// This method renders the list of items
void Display::render_item_list(bool is_new_list_set, bool navigation_direction, bool is_page_displayed, int item_count,
    ...
    if (is_new_list_set) {
        ...
        // Determine which nav button is pressed and if the current index is in the beginning/ending,
        // we know which part of the list should be shown (previous/next items)
    ...
    for (byte i = 0; i < 8; ++i) {
        ...
        if (item_index < 40) {
            if (screen_item_list_type_items[item_index] != "") {
                ...
                // Update accordingly the current screen list items array
    ...
    if (is_page_displayed) {
        ...
        // Item counted on the bottom right
...
//            //current_feature_item_type = LIST_ITEM;
        ...
        case RFID_FACTORY_SELECT: {
            ...
            // Clear selected items of screens which have the list
            for (byte i = 0; i < 10; ++i) {
        ...
        case RFID_BUYER_PO_LIST: {
            ...
            // Draw the list of items to be displayed
            int x_index = 32;
